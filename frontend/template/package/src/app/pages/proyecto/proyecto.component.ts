// Importaciones core de Angular
import { Component } from '@angular/core';
import { RouterModule } from '@angular/router';
import { CommonModule } from '@angular/common';
import { ProyectoService } from '../../services/ProyectoService';

// Importaciones de Angular Material para UI components
import { MatButtonModule } from '@angular/material/button';
import { MatCardModule } from '@angular/material/card';
import { MatIconModule } from '@angular/material/icon';
import { MatToolbarModule } from '@angular/material/toolbar';
import { MatFormFieldModule } from '@angular/material/form-field';
import { MatInputModule } from '@angular/material/input';
import { MatSelectModule } from '@angular/material/select';
import { MatDatepickerModule } from '@angular/material/datepicker';
import { MatNativeDateModule } from '@angular/material/core';
import { MatRadioModule } from '@angular/material/radio';
import { MatCheckboxModule } from '@angular/material/checkbox';
import { MatSlideToggleModule } from '@angular/material/slide-toggle';
import {MatTableDataSource, MatTableModule} from '@angular/material/table';
import { MatPaginatorModule } from '@angular/material/paginator';
import { MatSortModule } from '@angular/material/sort';
import { MatDialogModule } from '@angular/material/dialog';
import { MatSnackBarModule } from '@angular/material/snack-bar';
import { MatTooltipModule } from '@angular/material/tooltip';
import { MatSidenavModule } from '@angular/material/sidenav';
import { MatListModule } from '@angular/material/list';
import { MatMenuModule } from '@angular/material/menu';
import { MatTabsModule } from '@angular/material/tabs';
import { MatProgressBarModule } from '@angular/material/progress-bar';
import { MatProgressSpinnerModule } from '@angular/material/progress-spinner';

// Importaciones específicas para tipos de datos especiales
import { DatePipe } from '@angular/common';

/**
 * Componente principal para la gestión de la entidad Proyecto
 * Proporciona una interfaz para visualizar y manipular los datos de la entidad
 */
@Component({
  selector: 'app-proyecto',
  standalone: true,
  imports: [
    // Módulos básicos de Angular
    CommonModule,
    RouterModule,
    // Módulos de Angular Material
    MatButtonModule,
    MatCardModule,
    MatIconModule,
    MatToolbarModule,
    MatFormFieldModule,
    MatInputModule,
    MatSelectModule,
    MatDatepickerModule,
    MatNativeDateModule,
    MatRadioModule,
    MatCheckboxModule,
    MatSlideToggleModule,
    MatTableModule,
    MatPaginatorModule,
    MatSortModule,
    MatDialogModule,
    MatSnackBarModule,
    MatTooltipModule,
    MatSidenavModule,
    MatListModule,
    MatMenuModule,
    MatTabsModule,
    MatProgressBarModule,
    MatProgressSpinnerModule
  ],
  templateUrl: './proyecto.component.html',
  styleUrls: ['./proyecto.component.scss']
})
export class ProyectoComponent {
  // Atributos de la entidad con sus tipos correspondientes
  /** id - Campo de tipo number */
  id: number;

  /** nombre - Campo de tipo string */
  nombre: string;

  /** valorContrato - Campo de tipo number */
  valorContrato: number;

  /** tiempoContractual - Campo de tipo string */
  tiempoContractual: string;

  /** objetoContractual - Campo de tipo string */
  objetoContractual: string;

  /** alcanceContractual - Campo de tipo string */
  alcanceContractual: string;

  /** estado - Campo de tipo boolean */
  estado: boolean;

  /** numeroContrato - Campo de tipo string */
  numeroContrato: string;

  /** cliente - Campo de tipo string */
  cliente: string;

  /** fechaInicio - Campo de tipo Date */
  fechaInicio: Date;

  /** fechaFin - Campo de tipo Date */
  fechaFin: Date;

  /** creador - Campo de tipo string */
  creador: string;

  /** persona - Campo de tipo any[] */
  persona: any[];

  /** contrato - Campo de tipo any[] */
  contrato: any[];

  /** informe - Campo de tipo any[] */
  informe: any[];

  // Configuración de columnas para tablas de atributos y relaciones
  displayedColumnsAttributes: string[] = ['name', 'type'];
  displayedColumnsRelations: string[] = ['entity', 'type'];

  // DataSource para la tabla de atributos
  dataSourceAttributes = new MatTableDataSource([
    { name: 'id', type: 'long' },
    { name: 'nombre', type: 'String' },
    { name: 'valorContrato', type: 'long' },
    { name: 'tiempoContractual', type: 'String' },
    { name: 'objetoContractual', type: 'String' },
    { name: 'alcanceContractual', type: 'String' },
    { name: 'estado', type: 'boolean' },
    { name: 'numeroContrato', type: 'String' },
    { name: 'cliente', type: 'String' },
    { name: 'fechaInicio', type: 'LocalDate' },
    { name: 'fechaFin', type: 'LocalDate' },
    { name: 'creador', type: 'String' },
    { name: 'persona', type: 'Lista de Personas' },
    { name: 'contrato', type: 'Lista de Contratos' },
    { name: 'informe', type: 'Lista de Informes' },
  ]);

  // DataSource para la tabla de relaciones
  dataSourceRelations = new MatTableDataSource([
    { entity: 'Persona', type: 'Many-to-Many' },
    { entity: 'Contrato', type: 'One-to-Many' },
    { entity: 'Informe', type: 'One-to-Many' },
  ]);

  /**
   * Constructor del componente
   * @param service Servicio para la gestión de datos de la entidad
   */
  constructor(private service: ProyectoService) {
    // Inicialización de atributos con valores por defecto
    this.id = 0;
    this.nombre = '';
    this.valorContrato = 0;
    this.tiempoContractual = '';
    this.objetoContractual = '';
    this.alcanceContractual = '';
    this.estado = false;
    this.numeroContrato = '';
    this.cliente = '';
    this.fechaInicio = new Date();
    this.fechaFin = new Date();
    this.creador = '';
    this.persona = [];
    this.contrato = [];
    this.informe = [];
  }
}
