...
@Component({
  selector: 'app-crear-periodicidadpago',
  standalone: true,
  imports: [
    CommonModule,
    ReactiveFormsModule,
  ...
  templateUrl: './crear-periodicidadpago.component.html',
  styleUrls: ['./crear-periodicidadpago.component.scss']
})
export class CrearPeriodicidadPagoComponent implements OnInit {
  form = new FormGroup({});
  model: PeriodicidadPagoModel = {
    tipoPeriodoPago: '',
    creador: ''
  };
  ...
  constructor(
    private dialogRef: MatDialogRef<CrearPeriodicidadPagoComponent>,
  ...
  private saveEntity(modelData: any) {
    this.periodicidadPagoService.save(modelData).subscribe({
      next: (response) => {
        this.postCreate(response);
      },
      error: (error) => {
        console.error('Error al crear PeriodicidadPago:', error);
        this.snackBar.open('Error al crear PeriodicidadPago', 'Cerrar', {
          duration: 3000,
  ...
  postCreate(response: any) {
    this.snackBar.open('PeriodicidadPago creado exitosamente', 'Cerrar', {
      duration: 3000,
    });
