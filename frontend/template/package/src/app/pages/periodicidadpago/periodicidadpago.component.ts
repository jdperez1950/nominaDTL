...
/**
 * Componente principal para la gestión de la entidad PeriodicidadPago
 * Proporciona una interfaz para visualizar y manipular los datos de la entidad
 */
@Component({
  selector: 'app-periodicidadpago',
  standalone: true,
  imports: [
  ...
  templateUrl: './periodicidadpago.component.html',
  styleUrls: ['./periodicidadpago.component.scss']
})
export class PeriodicidadPagoComponent {
  ...
  /** tipoPeriodoPago - Campo de tipo string */
  ...
  dataSourceAttributes = new MatTableDataSource([
    { name: 'id', type: 'long' },
    { name: 'tipoPeriodoPago', type: 'String' },
   ...
   */
  constructor(private service: PeriodicidadPagoService) {
    // Inicialización de atributos con valores por defecto
    this.id = 0;
    this.tipoPeriodoPago = '';
    this.creador = '';
    this.contrato = [];
  }
}
