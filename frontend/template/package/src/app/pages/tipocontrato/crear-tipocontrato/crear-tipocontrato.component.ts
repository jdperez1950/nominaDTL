...
@Component({
  selector: 'app-crear-tipocontrato',
  standalone: true,
  imports: [
    CommonModule,
    ReactiveFormsModule,
  ...
  templateUrl: './crear-tipocontrato.component.html',
  styleUrls: ['./crear-tipocontrato.component.scss']
})
export class CrearTipoContratoComponent implements OnInit {
  form = new FormGroup({});
  ...
  constructor(
    private dialogRef: MatDialogRef<CrearTipoContratoComponent>,
  ...
  private saveEntity(modelData: any) {
    this.tipoContratoService.save(modelData).subscribe({
      next: (response) => {
        this.postCreate(response);
      },
      error: (error) => {
        console.error('Error al crear TipoContrato:', error);
        this.snackBar.open('Error al crear TipoContrato', 'Cerrar', {
          duration: 3000,
  ...
  postCreate(response: any) {
    this.snackBar.open('TipoContrato creado exitosamente', 'Cerrar', {
      duration: 3000,
    });
